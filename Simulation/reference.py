...
file.close()
def design(row,column,scale,Products,Position,Obstacles):
    details={}
    ...
    for i in range(0,2):
        if i==0:
            startx= 0*scale
            starty= row*scale-int(0*scale)-scale
            d.text((startx+(scale/2)-5,starty+int(scale/2)-10),  'Start' ,fill="Black", font=ImageFont.truetype("arial",22))
        else:
        ...
        if i==0:
            d.text((startx+(scale/4),starty+int(scale/2)-10),  'Start' ,fill="Black", font=ImageFont.truetype("arial",22))
        else:
            d.text((startx+(scale/4),starty+int(scale/2)-10),  'Billing' ,fill="Black", font=ImageFont.truetype("arial",22))
    ...
    for items in Products:
        if items in details:
            ...
            shape = [startx,starty,endx,endy] 
            if value[5]=='L':
                d.text((startx+5,starty+int(scale/2)-5), items,fill="Black", font=ImageFont.truetype("arial",14,))
                d.text((startx+(scale/2)-5,starty+int(scale/2)-10),  '|' ,fill="Black", font=ImageFont.truetype("arial",22))
            elif value[5]=='R':
                d.text((startx+(scale/2),starty+int(scale/2)-5),  items ,fill="Black", font=ImageFont.truetype("arial",14))
                d.text((startx+(scale/2)-5,starty+int(scale/2)-10),  '|' ,fill="Black", font=ImageFont.truetype("arial",22))
            elif value[5]=='U':
...
design(row,column,scale,Products,Position,Obstacles)
...
turtle.penup()
turtle.goto(-((column*scale//2)-scale//2),-((row*scale//2)-scale//2))
turtle.pendown()
turtle.listen()
dist = 100
def up():
...
for item in range(0,len(Products)):
    if Products[item]!='' and Position[item][-1]=='L':
        ...
        tempy+=1
        Details[Products[item]]=str(tempx)+","+str(tempy)
Details['billing']=str(column-1)+","+str(0)
print(Details)
'''Details={
    'Masks':'4,1',
    'Fruits':'1,3',
...
def algo(rem_goals,initial_co):
    ...
    for i in range(0,len(perm_set)):
        if [column-1,0] not in perm_set[i]:
            perm_set[i].append([column-1,0])
